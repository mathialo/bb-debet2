...
public class SalesHistory implements Iterable<Sale>, Exportable {
    ...
    /**
     * return a sale history as a obserable list of Sale's toString, prefixed by sale num
     *
    ...
    public ObservableList<String> toObservableList() {
        ...
        int i = 0;
        for (Sale s : history) {
            listOfTransactions.add(i + " " + s.toString());
            i++;
        }
     ...
     */
    public ObservableList<String> filterOnUser(User user) {
        // add all elements to arraylist
        ArrayList<String> listOfTransactions = new ArrayList<String>();

        for (Sale s : history) {
            if (s.madeByUser(user)) {
                listOfTransactions.add(s.toString());
            }
        }
